...
use risc0_zkvm::{
    compute_image_id_v2, default_executor, sha::Digestible, ExecutorEnv, FakeReceipt, InnerReceipt,
...
fn proves_one_step() {
    for length in 1..32 {
        ...
        env_builder
            .write(
                &GuestState::initial(compute_image_id_v2(SET_BUILDER_ID).unwrap())
...
fn proves_incremental() {
    for length in 1..16 {
        ...
        let mut state = GuestState::initial(compute_image_id_v2(SET_BUILDER_ID).unwrap());
