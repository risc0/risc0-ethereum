...
//! The [SetVerifierService] struct and related functionality for interacting with the
//! `RiscZeroSetVerifier` smart contract, implementing Merkle-tree based set-inclusion receipts as
//! implemented by [risc0_aggregation].
...
/// Struct for interacting with the `RiscZeroSetVerifier` contract, extending the functionality
/// provided by [IRiscZeroSetVerifierInstance].
#[derive(Clone)]
...
{
    /// Create a new [SetVerifierService] with the given address, provider, and caller address.
    ...
    /// Access the [IRiscZeroSetVerifierInstance], generated by `alloy`.
    ...
    /// Check whether the given aggregate receipt root is verified on the `RiscZeroSetVerifier`
    /// contract.
    ...
    /// Submit a root receipt for a [SetInclusionReceipt] to populate the cached root verification
    /// and allow for verification of leaf receipts via Merkle path verification.
    ...
    /// Verify the given receipt seal by calling the `RiscZeroSetVerifier` contract.
    ...
    /// Get the image ID and a download URL for the Set Builder guest used by this contract.
    ...
    ///
    /// This function searches through emitted event logs for the root seal from a verification
    /// call. It uses the default [EventQueryConfig].
    ///
    /// See [SetVerifierService::query_verified_root_event].
    pub async fn fetch_verified_root_seal(&self, root: B256) -> Result<Bytes> {
        self.query_verified_root_event(root, None, None).await
    }

    /// Get the latest block for the connected chain.
