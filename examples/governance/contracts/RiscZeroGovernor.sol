    ...
    GovernorVotesQuorumFraction
{
    ...
    /// `proposalId`: the proposal identifier to be finalized.
    /// `finalBallotBoxAccum` hash of the ballot inputs, which must match the proposal.
    /// `encodedBallots` a packed byte array containing the deduplicated list of validated
    ///  ballots is a 24-byte { uint8(support) || address } format. Note that it is provided as
